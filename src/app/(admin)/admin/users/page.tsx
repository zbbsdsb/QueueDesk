...
const ROLE_COLORS: Record<string, string> = {
  ...
  agent: "bg-cyan-100 text-cyan-700 dark:bg-cyan-900 dark:text-cyan-300",
  requester: "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400",
};
...
const STATUS_COLORS: Record<string, string> = {
  ...
  invited: "bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-300",
};
            ...
            {loading ? (
              ...
              filtered.map((user) => (
                <tr key={user.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium ${ROLE_COLORS[user.role] ?? ""}`}>
                      <ShieldCheck className="w-3 h-3" />
                      {user.role}
                    </span>
                  </td>
                  <td className="px-5 py-4">
                    <span className={`inline-flex px-2.5 py-1 rounded-lg text-xs font-medium ${STATUS_COLORS[user.status ?? "active"]}`}>
                      {user.status ?? "active"}
                    </span>
                  </td>
